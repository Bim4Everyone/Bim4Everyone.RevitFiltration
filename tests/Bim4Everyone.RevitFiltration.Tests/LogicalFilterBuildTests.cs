...
public class LogicalFilterBuildTests : RevitApiTest {
    ...
    private static Pipe CreatePipe(Document doc, Level level, int index) {
        ...
        return Pipe.Create(doc, systemTypeId, pipeTypeId, level.Id,
            new XYZ(index * 10.0, 0, 0), new XYZ(index * 10.0 + 5.0, 0, 0));
    }
    ...
    private static Duct CreateDuct(Document doc, Level level, int index) {
        ...
        return Duct.Create(doc, systemTypeId, ductTypeId, level.Id,
            new XYZ(index * 10.0, 20, 0), new XYZ(index * 10.0 + 5.0, 20, 0));
    }

    private static Wall CreateWallWithLength(Document doc, Level level, int index, double lengthFeet) {
        var startX = index * 30.0;
    ...
    private static Floor CreateFloorWithSize(Document doc, Level level, int index, double sizeFeet) {
        var offsetX = index * 50.0;
        const double offsetY = 200.0;
        var loop = new CurveLoop();
        loop.Append(Line.CreateBound(new XYZ(offsetX, offsetY, 0),                   new XYZ(offsetX + sizeFeet, offsetY, 0)));
        loop.Append(Line.CreateBound(new XYZ(offsetX + sizeFeet, offsetY, 0),        new XYZ(offsetX + sizeFeet, offsetY + sizeFeet, 0)));
        loop.Append(Line.CreateBound(new XYZ(offsetX + sizeFeet, offsetY + sizeFeet, 0), new XYZ(offsetX, offsetY + sizeFeet, 0)));
    ...
    private static Pipe CreatePipeWithLength(Document doc, Level level, int index, double lengthFeet) {
        ...
        var pipeTypeId = new FilteredElementCollector(doc).OfClass(typeof(PipeType)).First().Id;
        return Pipe.Create(doc, systemTypeId, pipeTypeId, level.Id,
            new XYZ(startX, 100.0, 0), new XYZ(startX + lengthFeet, 100.0, 0));
    }
    ...
    private static Duct CreateDuctWithLength(Document doc, Level level, int index, double lengthFeet) {
        ...
        var ductTypeId = new FilteredElementCollector(doc).OfClass(typeof(DuctType)).First().Id;
        return Duct.Create(doc, systemTypeId, ductTypeId, level.Id,
            new XYZ(startX, 200.0, 0), new XYZ(startX + lengthFeet, 200.0, 0));
    }
    ...
    public async Task FilteredCollect_Walls_BeginsWithRule_ReturnsExactCount() {
        ...
        try {
            ...
            var prefix = "WBW_" + Guid.NewGuid().ToString("N") + "_";
            var other  = "WOTH_" + Guid.NewGuid().ToString("N") + "_";
            RunInTransaction(document, "Create walls", () => {
                for (int i = 0; i < 3; i++) {
            ...
            var count = Collect(document, BuiltInCategory.OST_Walls, elementFilter);
    ...
    public async Task FilteredCollect_Walls_ContainsRule_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            var mid   = "_WC_" + Guid.NewGuid().ToString("N") + "_";
            var other = "WOTH_" + Guid.NewGuid().ToString("N");
            RunInTransaction(document, "Create walls", () => {
                for (int i = 0; i < 2; i++) {
                    var wall = CreateWall(document, level, i);
    ...
    public async Task FilteredCollect_Walls_EndsWithRule_ReturnsExactCount() {
        ...
        try {
            var level  = GetDefaultLevel(document);
            var suffix = "_WE_" + Guid.NewGuid().ToString("N");
            var other  = "WOTH_" + Guid.NewGuid().ToString("N");
            RunInTransaction(document, "Create walls", () => {
                for (int i = 0; i < 4; i++) {
                    var wall = CreateWall(document, level, i);
    ...
    public async Task FilteredCollect_Floors_BeginsWithRule_ReturnsExactCount() {
        ...
        try {
            var level  = GetDefaultLevel(document);
            var prefix = "FBW_" + Guid.NewGuid().ToString("N") + "_";
            var other  = "FOTH_" + Guid.NewGuid().ToString("N") + "_";
            RunInTransaction(document, "Create floors", () => {
                for (int i = 0; i < 4; i++) {
            ...
            var count = Collect(document, BuiltInCategory.OST_Floors, elementFilter);
            await Assert.That(count).IsEqualTo(4);
        } finally {
            document.Close(false);
        }
    ...
    public async Task FilteredCollect_Floors_ContainsRule_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            var mid   = "_FC_" + Guid.NewGuid().ToString("N") + "_";
            var other = "FOTH_" + Guid.NewGuid().ToString("N");
            RunInTransaction(document, "Create floors", () => {
                for (int i = 0; i < 3; i++) {
                    var floor = CreateFloor(document, level, i);
    ...
    public async Task FilteredCollect_Floors_EndsWithRule_ReturnsExactCount() {
        ...
        try {
            var level  = GetDefaultLevel(document);
            var suffix = "_FE_" + Guid.NewGuid().ToString("N");
            var other  = "FOTH_" + Guid.NewGuid().ToString("N");
            RunInTransaction(document, "Create floors", () => {
                for (int i = 0; i < 2; i++) {
                    var floor = CreateFloor(document, level, i);
    ...
    public async Task FilteredCollect_Pipes_BeginsWithRule_ReturnsExactCount() {
        ...
        try {
            var level  = GetDefaultLevel(document);
            var prefix = "PBW_" + Guid.NewGuid().ToString("N") + "_";
            var other  = "POTH_" + Guid.NewGuid().ToString("N") + "_";
            RunInTransaction(document, "Create pipes", () => {
                for (int i = 0; i < 4; i++) {
            ...
            var count = Collect(document, BuiltInCategory.OST_PipeCurves, elementFilter);
            await Assert.That(count).IsEqualTo(4);
        } finally {
            document.Close(false);
        }
    ...
    public async Task FilteredCollect_Pipes_ContainsRule_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            var mid   = "_PC_" + Guid.NewGuid().ToString("N") + "_";
            var other = "POTH_" + Guid.NewGuid().ToString("N");
            RunInTransaction(document, "Create pipes", () => {
                for (int i = 0; i < 3; i++) {
                    var pipe = CreatePipe(document, level, i);
    ...
    public async Task FilteredCollect_Ducts_BeginsWithRule_ReturnsExactCount() {
        ...
        try {
            var level  = GetDefaultLevel(document);
            var prefix = "DBW_" + Guid.NewGuid().ToString("N") + "_";
            var other  = "DOTH_" + Guid.NewGuid().ToString("N") + "_";
            RunInTransaction(document, "Create ducts", () => {
                for (int i = 0; i < 3; i++) {
            ...
            var count = Collect(document, BuiltInCategory.OST_DuctCurves, elementFilter);
            await Assert.That(count).IsEqualTo(3);
        } finally {
            document.Close(false);
        }
    ...
    public async Task FilteredCollect_Ducts_ContainsRule_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            var mid   = "_DC_" + Guid.NewGuid().ToString("N") + "_";
            var other = "DOTH_" + Guid.NewGuid().ToString("N");
            RunInTransaction(document, "Create ducts", () => {
                for (int i = 0; i < 4; i++) {
                    var duct = CreateDuct(document, level, i);
    ...
    public async Task FilteredCollect_Walls_LengthGreater_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create walls", () => {
                for (int i = 0; i < 3; i++) CreateWallWithLength(document, level, i, 20.0);
                for (int i = 3; i < 5; i++) CreateWallWithLength(document, level, i, 5.0);
            });
    ...
    public async Task FilteredCollect_Walls_LengthLess_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create walls", () => {
                for (int i = 0; i < 3; i++) CreateWallWithLength(document, level, i, 5.0);
                for (int i = 3; i < 5; i++) CreateWallWithLength(document, level, i, 20.0);
            });
    ...
    public async Task FilteredCollect_Walls_LengthGreaterOrEqual_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create walls", () => {
                for (int i = 0; i < 3; i++) CreateWallWithLength(document, level, i, 10.0);
                for (int i = 3; i < 5; i++) CreateWallWithLength(document, level, i, 5.0);
            });
    ...
    public async Task FilteredCollect_Floors_AreaGreater_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create floors", () => {
                for (int i = 0; i < 3; i++) CreateFloorWithSize(document, level, i, 15.0);   // 225 sqft each
                for (int i = 3; i < 5; i++) CreateFloorWithSize(document, level, i, 5.0);    // 25 sqft each
            });
    ...
    public async Task FilteredCollect_Floors_AreaLess_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create floors", () => {
                for (int i = 0; i < 3; i++) CreateFloorWithSize(document, level, i, 5.0);    // 25 sqft each
                for (int i = 3; i < 5; i++) CreateFloorWithSize(document, level, i, 15.0);   // 225 sqft each
            });
    ...
    public async Task FilteredCollect_Floors_AreaLessOrEqual_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create floors", () => {
                for (int i = 0; i < 3; i++) CreateFloorWithSize(document, level, i, 10.0);   // 100 sqft each
                for (int i = 3; i < 5; i++) CreateFloorWithSize(document, level, i, 20.0);   // 400 sqft each
            });
    ...
    public async Task FilteredCollect_Pipes_LengthGreater_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create pipes", () => {
                for (int i = 0; i < 3; i++) CreatePipeWithLength(document, level, i, 20.0);
                for (int i = 3; i < 5; i++) CreatePipeWithLength(document, level, i, 5.0);
            });
    ...
    public async Task FilteredCollect_Pipes_LengthLessOrEqual_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create pipes", () => {
                for (int i = 0; i < 3; i++) CreatePipeWithLength(document, level, i, 10.0);
                for (int i = 3; i < 5; i++) CreatePipeWithLength(document, level, i, 20.0);
            });
    ...
    public async Task FilteredCollect_Ducts_LengthGreaterOrEqual_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create ducts", () => {
                for (int i = 0; i < 3; i++) CreateDuctWithLength(document, level, i, 10.0);
                for (int i = 3; i < 5; i++) CreateDuctWithLength(document, level, i, 5.0);
            });
    ...
    public async Task FilteredCollect_Ducts_LengthLess_ReturnsExactCount() {
        ...
        try {
            var level = GetDefaultLevel(document);
            RunInTransaction(document, "Create ducts", () => {
                for (int i = 0; i < 3; i++) CreateDuctWithLength(document, level, i, 5.0);
                for (int i = 3; i < 5; i++) CreateDuctWithLength(document, level, i, 20.0);
            });
